...
static void test_diagonalExtrapolateMaxBounds(raw_t* dbm, cindex_t dim, const int32_t* max)
{
    cindex_t i, j;
    ...
    for (i = 1; i < dim; ++i) {
        for (j = 0; j < dim; ++j) {
            if (i != j && (dbm_raw2bound(DBM(0, i)) < -max[i] || dbm_raw2bound(DBM(0, j)) < -max[j] ||
    ...
    for (j = 1; j < dim; ++j) {
        ...
        if (dbm_raw2bound(DBM(0, j)) < -max[j]) {
            DBM(0, j) = (max[j] >= 0 ? dbm_bound2raw(-max[j], dbm_STRICT) : dbm_LE_ZERO);
        }
    }
    dbm_close(dbm, dim);
}

static void test_diagonalExtrapolateLUBounds(raw_t* dbm, cindex_t dim, const int32_t* lower, const int32_t* upper)
{
    cindex_t i, j;
    ...
    for (i = 1; i < dim; ++i) {
        for (j = 0; j < dim; ++j) {
            if (i != j && (dbm_raw2bound(DBM(i, j)) > lower[i] || dbm_raw2bound(DBM(0, i)) < -lower[i] ||
    ...
    for (j = 1; j < dim; ++j) {
        ...
        if (dbm_raw2bound(DBM(0, j)) < -upper[j]) {
            DBM(0, j) = (upper[j] >= 0 ? dbm_bound2raw(-upper[j], dbm_STRICT) : dbm_LE_ZERO);
        }
    }
    dbm_close(dbm, dim);
}
...
/* allocate a point
 */
static int32_t* allocPt(uint32_t dim) { return (int32_t*)malloc(dim * sizeof(int32_t)); }
