...
void init_xinfo(xinfo_t* xinfo)
{
    ...
    xinfo->window = XCreateWindow(xinfo->display, XRootWindow(xinfo->display, xinfo->screen), 200, 200, WIDTH, HEIGHT,
                                  5, xinfo->depth, InputOutput, xinfo->visual, CWBackPixel, &xinfo->attributes);
    XSelectInput(xinfo->display, xinfo->window, ExposureMask | KeyPressMask);
    xinfo->fontinfo = XLoadQueryFont(xinfo->display, "6x10");
    XAllocNamedColor(xinfo->display, DefaultColormap(xinfo->display, xinfo->screen), "black", &xinfo->color, &dummy);
    xinfo->gr_values.font = xinfo->fontinfo->fid;
    xinfo->gr_values.foreground = xinfo->color.pixel;
    xinfo->gr_context = XCreateGC(xinfo->display, xinfo->window, GCFont | GCForeground, &xinfo->gr_values);
...
void setcolor(xinfo_t* xinfo, const char* col)
{
    ...
    XAllocNamedColor(xinfo->display, DefaultColormap(xinfo->display, xinfo->screen), col, &xinfo->color, &dummy);
    xinfo->gr_values.foreground = xinfo->color.pixel;
    xinfo->gr_context = XCreateGC(xinfo->display, xinfo->window, GCFont | GCForeground, &xinfo->gr_values);
}

void draw_point(xinfo_t* xinfo, int x, int y) { XDrawPoint(xinfo->display, xinfo->window, xinfo->gr_context, x, y); }
...
{
    draw_line(xinfo, MARGIN + x1 * rx, HEIGHT - MARGIN - ry * y1, MARGIN + x2 * rx, HEIGHT - MARGIN - ry * y2);
}
...
void dbm_draw(xinfo_t* xinfo, const int* dbm, int dim, int x1, int x2, const char* dbmCol, int border, double scalex,
              double scaley)
{
    XPoint pt[6];
    char buf[32];
    ...
    pt[1].x = DBM(x1, x2) < dbm_LS_INFINITY ? MARGIN + scalex * ((DBM(x1, x2) >> 1) - (DBM(0, x2) >> 1)) : WIDTH;
    pt[1].y = pt[0].y;

    inf = DBM(x1, 0) < dbm_LS_INFINITY ? (DBM(x1, 0) >> 1) : max;
    pt[2].x = DBM(x1, 0) < dbm_LS_INFINITY ? MARGIN + (DBM(x1, 0) >> 1) * scalex : WIDTH;
    pt[2].y = DBM(x1, x2) < dbm_LS_INFINITY ? HEIGHT - MARGIN - (inf - (DBM(x1, x2) >> 1)) * scaley : HEIGHT - MARGIN;
    ...
    pt[5].x = pt[0].x;
    pt[5].y = DBM(x2, x1) < dbm_LS_INFINITY ? HEIGHT - MARGIN - ((DBM(x2, x1) >> 1) - (DBM(0, x1) >> 1)) * scaley : 0;
...
void fed_draw(xinfo_t* xinfo, int** dbm, int n, int dim, int x1, int x2, const char* dbmCol, int border, double scalex,
              double scaley)
{
    int i;
